...
const Validation_Schema = Yup.object().shape({
  amt:Yup.string().required('Required').matches(/^[0-9]+$/,"must only be digit"),
	...
	async function generateotp(){
	  ...
	  async function checkotp(){
        ...
        bodyFormData.append("uid",localStorage.getItem("user_id"));
				makePostRequest("/add/money", bodyFormData).then((response) => {
					if (response.data.status === "1") {
            makePostRequest("/transaction", bodyFormData).then((response) => {
              if (response.data.status === "1") {
...
>
	...
	<div class="input_container">
      ...
      placeholder="Enter Amount"/>
        {errors.amt && touched.amt ? <div className="errmsg">{errors.amt}</div> : null}
    </div>
    <div class="input_container">
    ...
    <div class="input_container">
      ...
      <input  class="input_field"  value={values.cardno}
    ...
    <div class="input_container">
      ...
      <div class="split">
      <input  class="input_field" value={values.exdate}
      ...
      <input class="input_field" value={values.cvv}
	...
	{(otp>0)?<>
	<div class="input_container">
      <label for="password_field" class="input_label">Please Enter Correct OTP</label>
      <div class="split">
      <input  class="input_field" type="text" value={eotp} onChange={(e)=>seteotp(e.target.value)}  placeholder="Please Enter Correct OTP"/>
      <button  class="btn"  onClick={call} >Generate OTP</button>
    </div>
    ...
    <button class="btn btn-primary" onClick={generateotp} >Checkout</button>}
   <ToastContainer/>
