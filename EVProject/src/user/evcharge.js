...
export default function EvCharge() {
  ...
  function FilterStations(){
    let bodyFormData = new FormData();
    makeGetRequest(`/view/evstations?state=${state}&city=${city}`, bodyFormData).then((response) => {
        if (response.data.status === "1") {
         setdata(response.data.data)
        }else{  
            setdata([])
...
async function BookSlot(){
  ...
  if(localStorage.getItem("balance")>200){
    ...
    makePostRequest("/booking/station", bodyFormData).then((response) => {
        if (response.data.status === "1") {
  ...
  return (
    ...
    <div className="cardb">
                <div className="pimage">
                    <img className="pimage1" src={process.env.PUBLIC_URL + `/assets/cars.jpg`} alt="img" /> 
                </div>
                <div className="data1">
                <span> <b>Station Name:</b><span className="textcol">{c.cs_name}</span></span>
                <span><b>Status:</b> <span  className="textcol">{(c.is_active==true)?<>Active</>:<>In-Active</>}</span></span>
                <span><b>Capacity:</b> <span  className="textcol">{c.capacity}</span></span>
                <span> <b>Location:</b><span  className="textcol">{c.cs_area+" "+c.cs_city+" "+c.cs_state}</span>
                    
                </span>
                </div>
                {(c.is_active==true)?<><button className="btn btn-outline-primary btn-sm mt-2" onClick={()=>handleClickOpen(c.cs_id,c.cs_name)} >
                Add to Calender
                </button></>:<><button className="btn" disabled >
                Currently Out Of Service
                </button></>}
                
                <a className="buttond" href={c.location} target="_blank"><span>Direction</span></a>
                
    </div>
    </Grid>
   
 )) :<center><h2 className="text-center">No Station is Available Near You</h2></center>
}</Grid>
    </div>
    <ToastContainer/>
...
<h5>Please fill up below details </h5>
            ...
            <Grid container spacing={3}>
        <Grid item xs={6}>
          <TextField  disabled value={"User ID: "+localStorage.getItem('user_id')} fullWidth/>
        </Grid> 
        <Grid item xs={6}>
          <TextField  disabled value={"EV Station Name: "+evname} fullWidth/>
        </Grid>
        <Grid item xs={6}> 
         <TextField  type="date" inputProps={{min:ndate }}   value={date} onChange={(e) => (setdate(e.target.value))}/>   
        </Grid>
        <Grid item xs={6}>
        <TextField
            value={time}
        ...
        <Grid item xs={6}>   
        <span>Note: Your account will be charged 200 rupees for the booking. </span>
        </Grid>
        <Grid item xs={6}> 
        <button className="buttond" onClick={BookSlot}>Book Now</button>
        </Grid>
        </Grid> 
        </div>
</div>
